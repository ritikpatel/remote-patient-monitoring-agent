...
def write_report(df: pd.DataFrame, task: Task) -> None:
    ...
    lines.append(
        ...
        "- **Simulating the instrument, not the patient.** `simulators/home_kit_stream.py` "
        "replays a MIMIC stay that genuinely deteriorated and simulates only what a home "
        "kit would see of it -- channel availability, cadence, noise, non-wear gaps. It "
        "replaced `morphing.py`, which synthesised a deterioration onto a healthy "
        "volunteer because that cohort contained no deterioration at all (E10); the "
        "physiology is now real and only the device is not.\n"
